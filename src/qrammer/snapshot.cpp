...
Snapshot::Snapshot(QString category)
{
...
QString Snapshot::getSnapshot()
{
    ...
    if (db.open()) {
        ...
        result += QString("%1%2")
                      .arg("Six-Monthly", -16)
                      .arg(QString::number(sixMonthCvrg * 100, 'f', 1))
                  + "%\n";
        result
            += QString("%1%2").arg("Biweekly", -16).arg(QString::number(biWeeklyCvrg * 100, 'f', 1))
               + "%\n";

        result += "• Miscellaneous\n";
        result += QString("%1%2")
                      .arg("Times Practiced", -16)
                      .arg(QString::number(timesPracticed, 'f', 1))
                  + "\n";
...
QString Snapshot::getComparison(bool detailed)
{
    ...
    if (db.open()) {
        ...
        query.prepare("SELECT COUNT(*) FROM knowledge_units WHERE category = :category AND "
                      "times_practiced > 0 AND previous_score >= passing_score AND is_shelved = 0");
        query.bindValue(":category", category);
        if (query.exec() && query.first())
